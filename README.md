...
----버전4.0----
■ BMS
1. 특정 노트에 이상한 시간이 들어감
 - int형 변수가 나눗셈으로 인해 0.5에서 소수점 자리 버려지고 있었다.
   
2. BMS 롱노트 파싱 및 구현의 문제
 - 서로 다른 마디에서 롱노트가 연결되는 경우가 있었다.
 - 현재 스크립트를 한 줄씩 읽어가면서 노트를 각 트랙에 배정하고 있는데 이 경우 문제가 됨.
 - 각 트랙에 해당하는 리스트를 만들어 노트정보들을 담은 후, 트릭별로 노트 배정을 해야함.
 - 좀 더 세분화해 BMS를 파싱하는 부분, 파싱한 데이터를 담은 리스트를 바탕으로 노트를 생성하는 함수를 따로 만들어서 테스트 중
   현재 콘솔 출력 정상, 배치는 아직 미구현.
